...
if (window.CRACK_HELPER_INJECTED) return;
...
const LIMIT = 10;
//dictionaries from some sad script guy, consider better ones at some point
const PASSWORD_DATABASE = {
    ...
    "Custom": "https://raw.githubusercontent.com/MK07/Torn-Cracking-Helper/main/dictionaries/Custom.txt" //is it possible to record to this?? future idea
};
...
let cracker_helper = { "source": "", "data": [] };
...
const fetch_action = (useCache = true) => {
    ...
    http_get(PASSWORD_DATABASE[CRACKER_SEL],
        (res) => {
            ...
            if (useCache) GM_setValue(CRACKER_HELPER_KEY, cracker_helper);
            setCrackTitle("Loaded");
            updatePage();
            console.log('load cracker_helper from network:', cracker_helper);
        },
...
const insertSelector = () => {
    ...
    selector.find("button").click(() => updatePage());
...
const addStyle = () => {
    const styles = `
        .cracker-helper-selector{ display: flex; align-items: center; font-size: 14px; font-weight: bold; }
        .cracker-helper-selector select{ background: transparent; text-align: center; border: none; }
        .dark-mode .cracker-helper-selector select{ color: #F2F2F2 !important; }
        .dark-mode .cracker-helper-selector select option{ background: #333 !important; color: #F2F2F2 !important; }
        .cracker-helper-panel{ width: 100%; height: 30px; background: #F2F2F2; box-sizing: border-box; display: flex; padding: 5px; border-bottom: 1px solid rgba(1, 1, 1, .1); }
        .cracker-helper-panel:hover{ background: #FFF; }
        .dark-mode .cracker-helper-panel{ background: rgba(1, 1, 1, .15) !important; border-bottom: 1px solid #222 !important; }
        .dark-mode .cracker-helper-panel:hover{ background: rgba(1, 1, 1, .15) !important; }
        .cracker-current-status{ display: flex; flex-flow: column nowrap; border-right: 1px solid; border-image-source: linear-gradient(180deg,transparent,#ddd 53%,transparent); border-image-slice: 1; box-sizing: border-box; justify-content: center; padding-left: 5px; }
        .dark-mode .cracker-current-status{ border-image-source: linear-gradient(180deg,transparent,#000,transparent); }
        .cracker-helper-panel-mobile .cracker-current-status{ font-size: 5px !important; }
        .cracker-current-status div{ width: 100%; color: #666; font: inherit; }
        .cracker-status-count{ color: #c66231 !important; }
        .cracker-current-result{ flex: 1; display: flex; align-items: center; border-right: 1px solid; border-image-source: linear-gradient(180deg,transparent,#ddd 53%,transparent); border-image-slice: 1; box-sizing: border-box; padding-left: 5px; font-size: 1.25em; }
        .dark-mode .cracker-current-result{ border-image-source: linear-gradient(180deg,transparent,#000,transparent); }
        .cracker-result-item{ border: 1px solid rgba(1, 1, 1, .1); height: 34px; line-height: 34px; font-size: 100%; text-align: center; margin-left: 6px; box-sizing: border-box; }
        .dark-mode .cracker-result-item{ border-color: #F2F2F266 !important; }
        .cracker-helper-panel-mobile .cracker-result-item{ margin-left: 3px !important; }
        .cracker-button-set{ display: flex; flex-flow: row nowrap; justify-content: space-between; align-items: center; box-sizing: border-box; padding-right: 5px; }
        .cracker-helper-panel-mobile .cracker-button-set{ flex-flow: column nowrap !important; }
        .cracker-button-set button{ text-align: center; height: 16px; line-height: 16px; }
        .cracker-button-set button:active{ height: 16px !important; line-height: 16px !important; }
        .cracker-helper-panel-mobile .cracker-button-set button{ width: 60px; height: 18px !important; line-height: 18px !important; }
    `;
    GM_addStyle(styles);
};
...
const findCommonCandidates = (arr, target) => {
    ...
    for (let i = 0; i < target.length; i++) {
        if (target[i] == ".") target_poses.push(i);
    }
    ...
    for (let pos of target_poses) {
        ...
        freqs.push({ pos: pos, char: freq_list[0][0], count: freq_list[0][1] });
    }
    ...
    for (let c of arr) {
        if (c[highest_pos] === highest_char) res.unshift(c);
        else res.push(c);
    }
    return { res, highest_pos, highest_char };
};
...
const handleCrime = (item, extraInfo = undefined, panel_index = undefined) => {
        ...
        while (!found && splitIndex < 7 && splitIndex < target.length - 1) {
            ...
            if (leftResult.length > 0 && rightResult.length > 0) {
                ...
                result = leftResult.map((item, index) => {
                    if (index < minSize) return item + rightResult[index];
                }).filter(item => item !== undefined);

                if (result.length > 10) found = true;
            }
        }
    }
    ...
    if (filter_history[index]) {
        result = result.filter((item) => {
            for (let history of filter_history[index]) {
                if (item && item[history.charPos] === history.char) return false;
            }
            return true;
        });
    }
    ...
    if (result.length > 0) {
        cracker_record[index || global_index] = result;
    }
    ...
    const updateResults = () => {
        ...
        item.find("div[class*=inline-crime-helper-elemet]").remove();
        ...
        if (result.length > 0) {
            ...
            item.find("div[class*=charSlot_]").each(function () {
                ...
                if (inputs.length > 0) {
                    inputs.each(function () {
                        $(this).text(result[result_index][i]).css('color', 'gray');
                    });
                } else if ($(this).text() == "") {
                    $(this).append($(`<div class="inline-crime-helper-elemet">${result[result_index][i]}</div>`)).css('color', 'gray');
                } else {
                    $(this).css('color', '');
                }
                i++;
    ...
    $(buttons_section).append(next_prev).css("grid-auto-flow", "row");
};

const handlePage = (crimes) => {
    $('.inline-crime-helper-elemet').remove();
    for (let i = 0; i < crimes.length; i++) {
        const target = initial_targets[i];
        handleCrime($(crimes[i]), undefined, target ? target["ID"] : undefined);
    }
};
...
const updatePage = () => {
    ...
    if (location.href.endsWith("cracking")) {
        ...
        $('.cracker-helper-panel, div.cracker-helper-selector').remove();
    }
};

const handleCrackPerpare = (params, data) => {
    const crimeValue = parseInt(params.get("value1"));
    if (!params.get("value2")) return;
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        if (target_index < 10000) {
            ...
            target_panel.find("button.cracker-button-prev, button.cracker-button-next").attr("data-attr", target_id);
        ...
        if (target_id === crimeValue) {
            ...
            if (currentChar !== char) {
                if (!filter_history[target_id]) filter_history[target_id] = [];
                filter_history[target_id].push({ char, charPos });
            ...
            for (let i = 0; i < targets.length; i++) {
                ...
                if (target_id === crimeValue) {
                    for (let j = 0; j < target.password.length; j++) {
                        ...
                        targetChars += (char === '*' || char === undefined) ? '.' : char;
                    }
                    setTimeout(() => { updatePage(); }, 300);
                }
            }
...
const interceptFetch = () => {
    ...
    targetWindow.fetch = async (...args) => {
        ...
        if (url.pathname === '/page.php' && params.get('sid') === 'crimesData') {
            ...
            if (step === "prepare") handleCrackPerpare(params, await clonedRsp.json());
            else if (step === "attempt") handleCrackAttempt(params, await clonedRsp.json());
            else if (step === "crimesList") handleCrackList(await clonedRsp.json());
        }
